...
Mpd = "/usr/bin/mopidy"  # Music Player Daemon via mopidy
...
class Radio:
	...
	def start(self):
		...
		self.execCommand("service mopidy start")
		# Connect to MPD
		self.boardrevision = self.getBoardRevision()
		self.mpdport = self.getMpdPort()
		log.message("radio.start before connect", log.DEBUG)
		self.connect(self.mpdport)
		log.message("radio.start after connect. client version = " + client.mpd_version, log.DEBUG)
		client.clear()
		log.message("radio.start after client.clear()", log.DEBUG)
		self.randomOff()
		log.message("radio.start after self.randomOff()", log.DEBUG)
		# self.randomOff()
		self.consumeOff()
		log.message("radio.start after self.consumeOff()", log.DEBUG)
		self.repeatOff()
		log.message("radio.start after self.repeatOff()", log.DEBUG)
		self.current_id = self.getStoredID(self.current_file)
	...
	def connect(self,port):
		...
		while retry > 0:
			...
			try:
				...
				log.message("Connected to MPD/MOPIDY port " + str(port), log.INFO)
				connection = True
				retry = 0
			except:
				log.message("Failed to connect to MPD/MOPIDY on port " + str(port), log.ERROR)
				time.sleep(0.5)	# Wait for interrupt in the case of a shutdown
				log.message("Restarting MOPIDY",log.DEBUG)
				if retry < 2:
					self.execCommand("service modipy restart")
				else:
					self.execCommand("service modipy start")
				time.sleep(20)	# Give MPD time to restart
				retry -= 1
	...
	def loadStations(self):
		...
		log.message("Playlists: \n %s"%self.execMpcCommand("lsplaylists"), log.ERROR)
		log.message("Playlist: \n %s"%self.execMpcCommand("playlist"), log.ERROR)
		self.randomOff()
		self.consumeOff()
		self.repeatOff()
	...
	def createPlayList(self):
		...
		while True:
			...
			log.message("radio.createPlaylist line %s "%line, log.DEBUG)
			if line.__len__() < 1:
				break
